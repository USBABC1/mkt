...
async function doRegisterRoutes(app: Express): Promise<HttpServer> {
    ...
    const authenticateToken = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
        if (process.env.FORCE_AUTH_BYPASS === 'true') {
            ...
            if (!user) {
              // Se o usuário de bypass não existir, crie um.
    ...
    // Rotas do WhatsApp
    apiRouter.get('/whatsapp/status', (req: AuthenticatedRequest, res) => res.json(getWhatsappServiceForUser(req.user!.id).getStatus()));
    ...
    apiRouter.post('/whatsapp/reload-flow', (req: AuthenticatedRequest, res) => { getWhatsappServiceForUser(req.user!.id).reloadFlow(); res.json({ message: "Recarga do fluxo solicitada." }); });
